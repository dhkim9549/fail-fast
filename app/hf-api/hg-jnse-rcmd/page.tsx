...
export default function RentLoanMultiInfo() {
  ...
  const [rentGrntAmt, setRentGrntAmt] = useState();
  const [trgtLwdgCd, setTrgtLwdgCd] = useState();
  const [age, setAge] = useState();
  const [weddStcd, setWeddStcd] = useState();
  const [myIncmAmt, setMyIncmAmt] = useState();
  const [myTotDebtAmt, setMyTotDebtAmt] = useState();
  const [ownHsCnt, setOwnHsCnt] = useState();
  const [grntPrmeActnDvcdCont01Yn, setGrntPrmeActnDvcdCont01Yn] = useState();
  ...
  return (
    <div className="">
      ...
      <div className="m-8 lg:mx-20">
        <Button variant="contained" size="large" onClick={setRcmdDataQuery}>
          조회
        </Button>
      </div>
