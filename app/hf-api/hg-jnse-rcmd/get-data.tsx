...
export async function getRcmdProdData(queryObj) {
  ...
  const rcmdItems = await getRcmdData(queryObj);

  const prodInfoObj = {};
  const p1 = Promise.all(
    rcmdItems.map(async (x) => {
  ...
  const loanRatObj = {};
  const p3 = getJnseGrtdLoanRatList(loanYm).then((items) => {
    items.map((x) => {
  ...
  const x = { rcmdItems, prodInfoObj, maxRentAmtObj, loanRatObj };
...
export async function getRcmdData(queryObj) {
  ...
  const res = await fetch(
    "https://apis.data.go.kr/B551408/jnse-rcmd-info-v2/jnse-rcmd-list-v2" +
      ...
      queryObj.weddStcd +
      "&myIncmAmt=" +
      (queryObj.myIncmAmt || "0") +
      "&myTotDebtAmt=" +
      (queryObj.myTotDebtAmt || "0") +
      "&ownHsCnt=" +
      (queryObj.ownHsCnt || "0") +
      "&grntPrmeActnDvcdCont=" +
      (queryObj.grntPrmeActnDvcdCont01Yn == "Y" ? "01" : ""),
...
export async function getJnseGrtdLoanRatList(loanYm) {
  ...
  const apiStr =
    ...
    "&dataType=JSON";
  const res = await fetch(
    "https://apis.data.go.kr/B551408/jnse-rcmd-info-v2/jnse-grtd-loan-rat-list-v2" +
      apiStr +
      "&loanYm=" +
      loanYm +
      "",
